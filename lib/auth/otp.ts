...
export async function isOtpLocked(email: string): Promise<boolean> {
  const valkey = await readyValkey()
  const attempts = Number((await valkey.get(attemptsKey(email))) ?? 0)
  return attempts >= 5
}
...
export async function verifyStoredOtp(
  ...
  const valkey = await readyValkey()
  const stored = await valkey.get(otpKey(email))
  const clean = code.replace(/\s/g, "")
  if (!stored || !/^\d{6}$/.test(clean) || !safeEqualHex(stored, hashOtp(clean))) {
    const attempts = await valkey.incr(attemptsKey(email))
    if (attempts === 1) {
      await valkey.expire(attemptsKey(email), LOCK_SECONDS)
    }
    if (attempts >= 5) return "locked"
    return "invalid"
  }
  await valkey.del(otpKey(email), attemptsKey(email))
  return "ok"
}
